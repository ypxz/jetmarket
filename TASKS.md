...
| QA-18 | bug | Operator can only send ONE quote per RFQ: `POST /api/quotes` after decline/withdraw hits unique `(rfq_id,operator_id)` → **500** raw constraint error (should be clean 409 or allowed re-quote). `e2e/lifecycle.api.spec.ts` pins current 500 — flip when fixed | fixed (PR pending) | `apps/web/app/api/quotes`, `packages/db` quotes unique |
...
| QA-21 | bug | Outbound emails (magic link, quote, deal, RFQ fan-out) reach Mailpit with **no `From:` header** — real SMTP providers will reject | fixed (PR pending) | `packages/providers` email adapter |
