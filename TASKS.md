...
| QA-9 | blocker | `/tos` and `/privacy` 500 "Something went wrong" — `legal.*` message keys missing; `sections.map is not a function` | fixed (PR#27) | `apps/web` legal pages, `packages/i18n` |
| QA-10 | bug | `/imprint` renders raw `legal.imprint.*` keys; entity/address/registration/contact all placeholders | fixed (PR#27) | `apps/web`, `packages/i18n` |
| QA-11 | bug | Year renders as "2,020" in public listing cards/detail (locale number grouping applied to year) | fixed (PR#27) | `apps/web` listing display |
...
| QA-16 | polish | Machinery operator dashboard still shows jets copy ("Create your first charter, empty leg or aircraft sale") | fixed (PR#27) | `apps/web` dashboard i18n per-vertical |
