...
| QA-15 | bug | `/app/listings/new` is jets-hardcoded (seats/model/from/to/date): under `VERTICAL=machinery` required attrs (machineryCategory/make/yearOfManufacture) have no inputs → UI listing creation impossible; e2e creates via API | fixed (PR#30) | `apps/web` listing form per-vertical |
