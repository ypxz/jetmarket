...
| QA-3 | bug | Mobile 390px viewport: operator header overflows — document width 554px; email + nav clip. Empty-leg date/model placeholders clip too | fixed (PR#15: shrink-0 nav, truncated email) | `apps/web` app shell/header |
| QA-4 | bug | Publish button has no busy state: double-click creates duplicate listings (repro: two identical "Fourth Charter" rows) | fixed (PR#15: pending state disables submit) | `apps/web` listing form |
...
| QA-7 | polish | No sign-out / account-switch control (must navigate to `/sign-in` manually) | fixed | `apps/web` app shell |
