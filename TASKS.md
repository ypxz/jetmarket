...
| QA-9 | blocker | `/tos` and `/privacy` 500 "Something went wrong" — `legal.*` message keys missing; `sections.map is not a function` | filed | `apps/web` legal pages, `packages/i18n` |
| QA-10 | bug | `/imprint` renders raw `legal.imprint.*` keys; entity/address/registration/contact all placeholders | filed | `apps/web`, `packages/i18n` |
| QA-11 | bug | Year renders as "2,020" in public listing cards/detail (locale number grouping applied to year) | filed | `apps/web` listing display |
...
| QA-17 | bug | Intermittent `SyntaxError: Unexpected end of JSON input` → 500 on `/app` in dev (e2e server logs); triggers Fast Refresh full-reloads that clear in-progress form state mid-session | fixed (PR pending) | `apps/web` — likely a client `JSON.parse`/`fetch` on `/api/vertical` or dashboard data |
